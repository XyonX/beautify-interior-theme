        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-sm p-4 shadow-sm border border-stone-100 sticky top-24">
              ...
              {/* Price Range */}
              <div className="mb-5">
                <h3 className="text-xs font-medium text-stone-800 mb-2">Price Range</h3>
              ...
              {/* Categories */}
              <div className="mb-5">
                <h3 className="text-xs font-medium text-stone-800 mb-2">Categories</h3>
                <div className="space-y-1.5">
                  {["Lighting", "Decor", "Wall Art", "Crafts", "Textiles", "Storage"].map((category) => (
                    <div key={category} className="flex items-center space-x-2">
                      ...
                      />
                      <Label htmlFor={category} className="text-xs text-stone-700">
                        {category}
                      </Label>
                    </div>
                  ))}
              ...
              {/* Colors */}
              <div className="mb-5">
                <h3 className="text-xs font-medium text-stone-800 mb-2">Colors</h3>
                <div className="space-y-1.5">
                  {["Natural", "White", "Gold", "Brown", "Blue", "Black", "Multi"].map((color) => (
              ...
              <Button className="w-full bg-stone-800 hover:bg-stone-700 text-xs rounded-sm h-8">Apply Filters</Button>
            </div>
          </div>
          ...
          <div className="lg:col-span-4">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
              <div>
                <h1 className="text-lg font-medium text-stone-800 mb-1">All Products</h1>
                <p className="text-xs text-stone-600">Showing {products.length} results</p>
              </div>
              ...
              {products.map((product) => (
                ...
                >
                  <div className={`${viewMode === "list" ? "flex" : ""}`}>
                    <div
                      className={`relative ${viewMode === "list" ? "w-40 flex-shrink-0" : "aspect-square"} overflow-hidden`}
                    >
                      <Image
                      ...
                      <div className="absolute top-2 left-2 flex flex-col gap-1">
                        {product.isNew && (
                          <Badge className="bg-stone-800 text-white text-xs px-1.5 py-0.5 rounded-sm">New</Badge>
                        )}
                        {product.isSale && (
                          <Badge className="bg-stone-700 text-white text-xs px-1.5 py-0.5 rounded-sm">Sale</Badge>
                        )}
                      </div>
                      <Button
                        size="sm"
                    ...
                    <CardContent className={`p-3 ${viewMode === "list" ? "flex-1" : ""}`}>
                      ...
                      <div className="flex items-center mb-2">
                        <div className="flex items-center">
                          ...
                          <span className="text-xs text-stone-600 ml-1">{product.rating}</span>
                        </div>
                      </div>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          <span className="text-xs font-medium text-stone-800">${product.price}</span>
                        ...
                        {viewMode === "list" && (
                          <Link href={`/products/${product.id}`}>
                            <Button size="sm" className="bg-stone-800 hover:bg-stone-700 h-7 text-xs rounded-sm">
                              View
                            </Button>
                          </Link>
                        )}
            ...
            <div className="flex justify-center mt-8">
              <div className="flex items-center space-x-1">
                <Button variant="outline" size="sm" disabled className="h-7 w-7 p-0 text-xs rounded-sm">
                  &lt;
                </Button>
                <Button size="sm" className="bg-stone-800 text-white h-7 w-7 p-0 text-xs rounded-sm">
                  1
                </Button>
                <Button variant="outline" size="sm" className="h-7 w-7 p-0 text-xs rounded-sm">
                  2
                </Button>
                <Button variant="outline" size="sm" className="h-7 w-7 p-0 text-xs rounded-sm">
                  3
